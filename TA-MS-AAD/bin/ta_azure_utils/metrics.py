...
def _cache_metric_definitions(helper, access_token, resource_id, resource_type):
    ...
    helper.log_debug("_Splunk_ Getting metric definitions from Azure REST for type '%s'" % resource_type)
    ...
    if len(metrics) > 0:
        ...
        helper.log_debug("_Splunk_ Saving metric definitions from Azure REST for type '%s'.  Definitions: %s" % (resource_type, str(metrics)))
...
def _get_metric_definitions_for_resource(helper, access_token, resource_id, resource_type):
    ...
    if metric_definitions is not None:
        helper.log_debug("_Splunk_ Found metric definitions for namespace '%s' in the cache." % resource_type)
    else:
        ...
        helper.log_debug("_Splunk_ Metric defininitions for namespace '%s' were not found in the cache.  Polling Azure REST for the metric definitions." % check_point_key)
        _cache_metric_definitions(helper, access_token, resource_id, check_point_key)
        metric_definitions = helper.get_check_point(check_point_key)
        if metric_definitions is None:
            helper.log_debug("_Splunk_ No metric definitions found for namespace '%s'. Skipping..." % check_point_key)
            return
    ...
    if metrics_last_updated < utc_now - datetime.timedelta(days=30):
        helper.log_debug("_Splunk_ Metric definitions may be out of date.  The last update for namespace '%s' was on '%s'. Polling Azure REST for the definitions." % (resource_type, str(metrics_last_updated)))
...
def _index_resource_metrics(helper, ew, access_token, resource_obj, metric_timespan, preferred_time_aggregation, requested_metric_statistics):
    ...
    for metric_list in metric_list_preferred:
        ...
        helper.log_debug("_Splunk_ Preferred metric URL: %s" % metric_url)
    ...
    for metric_list in metric_list_alternate:
        ...
        helper.log_debug("_Splunk_ Alternate metric URL: %s" % metric_url)
        _index_metrics(helper, ew, access_token, resource_obj, metric_url, requested_metric_statistics, metric_aggregations)
